...
namespace eilang.ArgumentBuilders
{
    public class ArgumentListBuilderWithParams
    {
        ...
        public ArgumentListBuilderWithParams(IValue value, string function, List<IArgumentValidator> arguments)
        {
            _value = value;
            _function = function;
    ...
    {
        private readonly List<IValue> _paramsArguments;

        public ParamsArgument(List<IValue> paramsArguments)
        {
        ...
        public T Get<T>()
        {
            if (typeof(T) != typeof(List<IValue>))
            {
                throw new InvalidCastException("It is only possible to get a params argument by using List<IValue> as the generic type parameter."); // ouff, this is bad, but it'll do for now
            }
