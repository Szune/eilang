...
public class Parser
{
    ...
    private readonly Stack<VariableScope> _scopes = new();
    private Token _lastConsumed = new();
    ...
    {
        var parsedDots = ParseDots();
        if (parsedDots is not AstMultiReference multi)
        {
            if (Match(TokenType.Equals))
            {
                ThrowParserException("Cannot assign to 'me'.");
            }
            else
            {
                ThrowParserException("'me' statement is only usable with member references, e.g. 'me.fieldX'.");
            }
            return;
        }
